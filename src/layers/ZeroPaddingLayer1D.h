#if !defined(ENN_ZERO_PADDING_LAYER_1D_H)
#define ENN_ZERO_PADDING_LAYER_1D_H
...
///
/// This layer adds padding of zeroes for width dimension
///
template <typename T = ENN_DEFAULT_TYPE,
					typename T_SIZE = ENN_DEFAULT_SIZE_TYPE>
class ZeroPaddingLayer1D : public LayerBase<T, T_SIZE> {
...
public:
	...
	virtual void forward()
	{
		for (T_SIZE i = 0; i < this->inputs().depth(); i++) {
			...
			T * dst = this->inputs().data(i) + _padding;
	...
	virtual void backward(T_INPUT& gradients)
	{
		for (T_SIZE i = 0; i < this->gradients().depth(); i++) {
			T * src = gradients.data(i) + _padding;
