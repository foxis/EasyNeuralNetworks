...
public:
	...
	///
	///
	LayerBase(const T* inputs, T_SIZE num_inputs, T* outputs, T_SIZE num_outputs, const ActivationBase<T>& activation)
	...
	///
	///
	LayerBase(const LayerBase &input, T* outputs, T_SIZE num_outputs, const ActivationBase<T>& activation)
		: _activation(activation) {
	...
	///
	virtual void backwards(const T * errors) = 0;
	...
	///
	///  Wij = W_arr(i + j * N)
	inline void calc_dense(const T * weights) {
		T acc;
		T_SIZE i, j;
		const T * in_p;
		const T * w_p;
		...
		for (j = 0, out_p = _outputs, w_p = weights; j < _num_outputs; j++) {
			acc = 0;
